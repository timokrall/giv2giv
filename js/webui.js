	...
	function startApplication(callback) {
		...
		if (activeSession()) {
			...
			$.ajax({
				...
				if (window.location.hash.indexOf("signin")!=-1 || window.location.hash.indexOf("signup")!=-1 || window.location.hash  === "#signin" || window.location.hash === "#signup" || window.location.hash === "" || window.location.hash === "#/") {
...
*/
	...
	// Landing Page Route
	crossroads.addRoute("landing", function() {
		var navTab = activeSession() ? $("#landing-nav") : $("#pub-landing-nav");
		loadPage("/ui/landing.html", function() {
			$("#app-container").attr("data-page-id", "landing");
			setPageMetadata(!activeSession(), navTab, "giv2giv");
			LandingUI.start.dispatch(); // Load JS
		});
	});
	...
	// Endowment Filtered List Route
	crossroads.addRoute("/endowments", function() {
		var navTab = activeSession() ? $("#endowments-nav") : $("#pub-endowments-nav");
		loadPage("/ui/endowments.html", function() {
			$("#app-container").attr("data-page-id", "endowments");
			setPageMetadata(!activeSession(), navTab, "giv2giv - Endowment List");
			EndowmentsUI.start.dispatch(); // Load JS
		});
	});
	...
	// FAQ Route
	crossroads.addRoute("/FAQ", function() {
		var navTab = activeSession() ? $("#FAQ-nav") : $("#pub-FAQ-nav");
		loadPage("/ui/FAQ.html", function() {
			$("#app-container").attr("data-page-id", "FAQ");
			setPageMetadata(!activeSession(), navTab, "giv2giv - Frequently Asked Questions");
		});
	});
	...
	// Not found route - send to landing page
